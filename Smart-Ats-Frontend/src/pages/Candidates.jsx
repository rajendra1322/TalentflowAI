    ...
    <>
      ...
      <div className="bg-white p-3 sm:p-4 rounded-2xl shadow mb-6 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-2 sm:gap-3 items-center">
        ...
        <button
          ...
          className="bg-slate-900 text-white rounded-lg py-1 px-3 sm:py-2 sm:px-4 text-sm"
        >
          Clear Filters
        </button>
      ...
      {(["recruiter", "hiringmanager", "admin"].includes(role)) ? (
        ...
        <div className="bg-white p-4 rounded-2xl shadow">
          ...
          {candidates.filter(c => (c.email || "").trim().toLowerCase() === userEmail).length === 0 ? (
            <p className="text-sm text-slate-500">No applications found for your account.</p>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {candidates
                 .filter(c => (c.email || "").trim().toLowerCase() === userEmail)
                .map(c => (
                  <div key={c._id} className="border rounded-lg p-4 hover:shadow-md transition">
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <div className="font-semibold text-sm">{c.name}</div>
                        <div className="text-xs text-slate-500 mt-1">{c.jobId?.title || c.role}</div>
                      </div>
                      <div className="text-sm font-medium text-sky-600 ml-3">{c.status}</div>
                    </div>
                    {(!c.skills || c.skills.length === 0) && (
                      <div className="text-xs text-slate-400 mt-3">No skills extracted</div>
                    )}
                  </div>
                ))}
            </div>
